...
# repo root = .../vectra_quant/  (contains config/, backend/, pwa/)
ROOT = Path(__file__).resolve().parents[2]
PARAMS_PATH = Path(os.getenv("VECTRA_QUANT_PARAMS", ROOT / "config" / "params.yaml"))
# Factory defaults, never written to. "Reset all" restores params.yaml from here.
DEFAULTS_PATH = Path(
    os.getenv("VECTRA_QUANT_PARAMS_DEFAULT", PARAMS_PATH.with_name("params.default.yaml"))
)
...
def _build(raw: dict[str, Any], secrets: Secrets) -> Settings:
    ...
    risk = RiskConfig(
        capital=float(raw.get("capital", 120_000)),
        target_pct=float(daily.get("target_pct", 0.08)),
        loss_limit_pct=float(daily.get("loss_limit_pct", 0.05)),
        risk_per_trade_pct=float(daily.get("risk_per_trade_pct", 0.025)),
        ...
        t2_protect_pct=float(protect.get("floor_pct", 0.50)),
        t3_trail_giveback_pct=float(trail.get("giveback", 0.25)),
