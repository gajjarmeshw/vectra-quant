"""
VECTRA_QUANT Risk Engine — deterministic core.
...
class RiskConfig:
    ...
    target_pct: float = 0.08              # 8% of capital = 9600
    loss_limit_pct: float = 0.05          # 5% of capital = 6000
    risk_per_trade_pct: float = 0.025     # 2.5% of capital = 3000
    base_trades: int = 2
    bonus_at_pct_target: float = 0.45     # realized >= 45% T -> 4th trade
    consecutive_loss_stop: int = 2
    t1_keep_pct: float = 0.70             # "hits 60 keep 70"
    t2_protect_pct: float = 0.50          # "hits target, keep 50%"
    t3_trail_giveback_pct: float = 0.25   # "above 1.3T, give back 25% of peak"

    @property
    def target(self) -> float:
        return self.capital * self.target_pct

    @property
    def loss_limit(self) -> float:
        return self.capital * self.loss_limit_pct
    ...
    def _refresh(self, mark_unrealized: float) -> EngineEvent:
        ...
        if self.state in (DayState.NORMAL, DayState.EARNED) and self.day_pnl >= c.protect_at * c.target:
            ...
            if not self.bonus_unlocked:          # target can jump past 45% gate
                self.bonus_unlocked = True
            self.floor = max(self.floor, c.t2_protect_pct * c.target)
        ...
        if self.state == DayState.TRAIL:
            self.floor = max(self.floor, (1 - c.t3_trail_giveback_pct) * self.peak)
